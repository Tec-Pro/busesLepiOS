...
class CambiarPassViewController: UIViewController{
    ...
    func passOk()-> Bool{
        return (txtPass.text == txtRepPass.text)
    }
    ...
    @IBAction func guardar(sender: AnyObject) {
        if dataOk(){
            if passOk(){
                ...
                var pass = preferences.valueForKey("pass") as! String
                ...
                var task = session.dataTaskWithRequest(lobj_Request, completionHandler: {data, response, error -> Void in
                    ...
                    if let rangeFrom = parser.rangeOfString(">1</return>") { // con esto hago un subrango
                        
                        dispatch_async(dispatch_get_main_queue(), { () -> Void in
                            //self.load.hidden = true
                            var alert = UIAlertView( title: "Atencion!", message: "Se cambio correctamente la contraseña",delegate: nil,  cancelButtonTitle: "Aceptar")
                            alert.show()
                            preferences.setValue(repPass, forKey: "pass")
